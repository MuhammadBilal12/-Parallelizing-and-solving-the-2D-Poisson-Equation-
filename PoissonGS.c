/* LAB COURSE II Project WS 18/19 - Gauss Seidel Method for solving Poisson Equation in MPI.

Code for Gauss Seidel without Successive Over-Relaxation.
Submitted by:
...
void GS_RED(struct mylattice *me, double h);
void GS_BLACK(struct mylattice *me, double h);
...
do{
	...
	copyPhi(&me);
	MPI_Allreduce(&localmax, &globalmax, 1, MPI_DOUBLE, MPI_MAX, comm_2D);
...
------------------------------------------------------------------------------------------------------------------------------------------------------------*/
printf("Total Iterations(Gauss Seidel) = %d\nTotal Grid Points = %d\nTotal Time(Gauss Seidel) = %f\nTotal Processes: %d\n", iterations, N, time, me.totalprocs);
...
for (i = 1; i<=me->localrows; i++){
	for(j = 1; j<=me->localcols; j++){
		if((i + j) % 2 == 0){
		me->phi[i*(me->localcols+2) + j] = 0.25*(me->phi[(i+1)*(me->localcols+2) + j] + me->phi[(i-1)*(me->localcols+2) + j] +  
